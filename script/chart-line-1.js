...
chart_1.dataSource.url = "https://raw.githubusercontent.com/CharlotteZKHu/hiv_twitter.github.io/main/data/hiv_test_longitudinal.csv";
...
// Create Series
function createSeries_self_1(field, name, bullet, cover) {
  ...
  var interfaceColors_1 = new am4core.InterfaceColorSet();
  ...
  switch(bullet) {
    case "triangle":
      ...
      triangle.stroke = interfaceColors_1.getFor("background");
    ...
    case "rectangle":
      ...
      rectangle.stroke = interfaceColors_1.getFor("background");
    ...
    default:
      ...
      bullet.circle.stroke = interfaceColors_1.getFor("background");
...
//     createSeries_self_1(res[2].substring(0, 2), res[2].substring(0, 2), "circle", false);
//   });
// }
...
function searchFunction_self() {
  ...
  tr_1 = table_1.getElementsByTagName("tr_1");
  for (i_1 = 0; i_1 < tr_1.length; i_1++) {
    td_1 = tr_1[i_1].getElementsByTagName("td_1")[0];
    td2_1 = tr_1[i_1].getElementsByTagName("td_1")[1];
    // td3 = tr[i_1].getElementsByTagName("td")[2];
    if (td_1) {
    ...
    if (td2_1) {
      txtValue_1 = td2_1.textContent || td2_1.innerText;
      if (txtValue_1.toUpperCase().indexOf(filter_1) > -1) {
        tr_1[i_1].style.display = "";
      }
    }
    if (td3_1) {
      txtValue_1 = td3_1.textContent || td3_1.innerText;
      if (txtValue_1.toUpperCase().indexOf(filter_1) > -1) {
        tr_1[i_1].style.display = "";
      }
    }
...
if (table_2 != null) {
    for (var i_1 = 1; i_1 < table_2.rows.length; i_1++) {
        for (var j_1 = 0; j_1 < table_2.rows[i_1].cells.length; j_1++)
        table_2.rows[i_1].cells[j_1].onclick = function () {
            tableText_self(this);
            this.onclick=null;
        };
    }
...
//     createSeries_self_1(location.substring(0, 2), location, "circle", false);
//   }
//   // check for region (triangle)
//   else if (regions.indexOf(location) > -1) {
//     createSeries_self_1(location, location, "triangle", false);
//   }
//   // otherwise a city (rectangle)
//   else {
//     createSeries_self_1(location, location, "rectangle", false);
//   }
// }
...
if (table_1 != null) {
    for (var i_1 = 1; i_1 < table_1.rows.length; i_1++) {
        for (var j_1 = 0; j_1 < table_1.rows[i_1].cells.length; j_1++)
        table_1.rows[i_1].cells[j_1].onclick = function () {
            tableText_self(this);
            this.onclick=null;
        };
    }
...
function tableText_self(tableCell) {
  var location_1 = tableCell.innerHTML;
  var prep_key_words_1 = ["test", "tests", "tested", "testing", "oraquick"];
  // check for state (circle)
  if (location_1.substring(0, 2) === location_1.substring(0, 2).toUpperCase()) {
    createSeries_self_1(location_1.substring(0, 2), location_1, "circle", false);
  }
  // check for region (triangle)
  else if (prep_key_words_1.indexOf(location_1) > -1) {
    createSeries_self_1(location_1, location_1, "triangle", false);
  }
  // otherwise a city (rectangle)
  else {
    createSeries_self_1(location_1, location_1, "rectangle", false);
  }
}


// plot US avg by default
createSeries_self_1("United States", "USA", "triangle", false);
